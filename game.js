...
var karp = new Audio("karp.mp3");
var ninja = new Audio("ninja.mp3");
var songLoopBeta = new Audio("songLoopBeta.mp3");
var derpSong = new Audio("derpSong.mp3");

var background = "background.jpg";

// Sets background
if(localStorage.pageLoadCount < 0){
    document.getElementById("game").style.backgroundImage = "url('pond.jpg')";
}
if(localStorage.pageLoadCount >= 10){
    document.getElementById("game").style.backgroundImage = "url('squirt.jpg')";
}


// Default 0 is pikachu. 1 = ninja, 2 = bulb, 3 = karp, 4 = true viking
var crt = 0;

var skin = "RedBeard.png";
...
function initAll()
{
    ...
    // Pikchu Theme
    if(localStorage.pageLoadCount >=0 && localStorage.pageLoadCount < 10){
    ...
    //Ninja Theme
    if(localStorage.pageLoadCount >= 10) {
        songLoopBeta.addEventListener('ended', function () {
            this.currentTime = 0;
            this.play();
        }, false);
        songLoopBeta.play(); // Starts game song.
    }
    //Bulba and Karp Theme
    if(localStorage.pageLoadCount < 0) {
        derpSong.addEventListener('ended', function () {
            this.currentTime = 0;
            this.play();
        }, false);
        derpSong.play(); // Starts game song.
    }
    canvasElem = document.getElementById("game");
    world = boxbox.createWorld(canvasElem);


    if(localStorage.pageLoadCount >= 15){
        alert("True Viking!");
        crt = 4;
        skin = "RedBeard.png";
        ya = karp;
    ...
    if(localStorage.pageLoadCount < 0 && localStorage.pageLoadCount > -10){
        alert("It seems your score is too low to select a warrior. Have a bulbba.");
        skin = "dur.png";
        ya = derp;
        crt = 2;
    ...
    if(localStorage.pageLoadCount <= -10){
        alert("You have reached a new low. Karp is too confused to run.");
        skin = "karp.png";
        ya = karp;
        crt = 3;
    }
    ...
    Pik.onKeydown(function(e) {
        ...
        //POWER ATTACK Right
        if(e.keyCode === 69 && crt !== 3){
            ya.play();
            this.applyImpulse(230, 90);
            return false;
        }
        // POWER ATTACK Left
        if(e.keyCode === 81 && crt !== 3){
            ya.play();
